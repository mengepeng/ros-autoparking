...
SearchParkingSpace::SearchParkingSpace(ros::NodeHandle* nodehandle):nh_(*nodehandle)
{
    ...
    sub_apa_ = nh_.subscribe<sensor_msgs::Range>("apa_rb", 1, \
...
int main(int argc, char **argv)
{
    ...
    SearchParkingSpace searchParkingSpace_rb(&nh_c);  // pass nh_c to class constructor
    ...
    while (ros::ok())
    {
        if (parking_enable)
        {
            ...
            if (trigger_check)
            {
                ...
                searchParkingSpace_rb.check_parking_space();
            }
        }
        else
        {
